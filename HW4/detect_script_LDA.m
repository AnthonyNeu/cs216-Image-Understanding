...
numNeg = 10; % number of negative example
...
lambda = 0.4;
covNeg = zeros(templateWidth*templateHeight*9,templateWidth*templateHeight*9);
% compute corivance matrix for negative template
for i = 1:numNeg
    reshapeTemplate = reshape(negativeTemplate(:,:,:,i), [templateWidth*templateHeight*9,1]);
    templateMean = repmat(mean(reshapeTemplate),[templateWidth*templateHeight*9,1]);
    covNeg =  covNeg + (reshapeTemplate - templateMean) *  (reshapeTemplate - templateMean)';
end
covNeg = covNeg/numNeg + lambda * eye(size(covNeg));

% for simple center model
template = positiveTemplateSum/numPos - negativeTemplateSum/numNeg;

% for Gaussian model
WHOtemplate = inv(covNeg) * reshape(template,[templateWidth*templateHeight*9 , 1]);
WHOtemplate = reshape(WHOtemplate,[templateHeight,templateWidth,9]);

% visualize the HOG template
% figure;
% V1 = hogdraw(template);
% imshow(V1);
figure;
V2 = hogdraw(WHOtemplate);
imshow(V2);
...
ndet = 5;
[x,y,score] = detect(Itest,WHOtemplate,ndet);
